#!/usr/bin/env python3
# main.py - Telegram + Flask + trading threads (Bitget Futures swap via ccxt)
import os
import time
import json
import logging
...
# Popular symbols (defaults) - user can add/remove via Telegram
SYMBOLS = [
...
def init_exchange():
    ...
    if BITGET_API_KEY and BITGET_API_SECRET:
        try:
            ...
            logging.info("Bitget swap initialized.")
...
# Helper to try to get some chat id from recent updates if TG_CHAT_ID not set
_last_chat_id = None
            ...
            try:
                ...
                if direction:
                    ...
                    if TRADE_MODE == "virtual" or exchange is None:
                        ...
                        try:
                            # best-effort leverage set (may be noop depending on exchange)
...
def panel_cmd(update: Update, context: CallbackContext):
    ...
    if row:
        kb.append(row)
    kb.append([InlineKeyboardButton("Virtual", callback_data="mode_virtual"), InlineKeyboardButton("Real", callback_data="mode_real")])
