        ...
        if text == "/start":
            ...
            if len(parts) >= 2:
                try:
                    ...
                    INVEST_AMOUNT = amt
                    save_settings()
                    send_message(chat_id, f"Размер сделки изменён на {INVEST_AMOUNT}$")
                except Exception:
                    send_message(chat_id, "Неверный формат. Используй: /amount 25")
            else:
                send_message(chat_id, f"Текущий размер сделки: {INVEST_AMOUNT}$")
        elif text == "/open":
            texts = ["Открытые сделки:"]
            for t in open_trades:
                texts.append(f"{t['id']} {t['symbol']} {t['direction']} entry={t['entry_price']}")
            send_message(chat_id, "\n".join(texts) if len(texts) > 1 else "Нет открытых сделок.")
        elif text == "/closed":
            texts = ["Закрытые сделки:"]
            for t in closed_trades[-20:]:
                texts.append(f"{t['id']} {t['symbol']} {t['direction']} PnL={t.get('pnl_percent')}")
            send_message(chat_id, "\n".join(texts) if len(texts) > 1 else "Нет закрытых сделок.")
        elif text == "/balance":
            vb = virtual_balance
            send_message(chat_id, f"Баланс виртуал: total={vb['total']}$, available={vb['available']}$")
        else:
            send_message(chat_id, "Команда не распознана. Введите /help для списка команд.")
...
def signal_checker_loop():
    while True:
        try:
            ...
            logging.error(f"Ошибка в check_signals_once loop: {e}")
...
def monitor_loop():
    while True:
        try:
            ...
            logging.error(f"Ошибка в monitor loop: {e}")
        time.sleep(30)

# Start background threads
t1 = threading.Thread(target=signal_checker_loop, daemon=True)
