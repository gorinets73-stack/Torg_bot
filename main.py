        ...
        for tf in list(ACTIVE_TF):
            try:
                if TRADE_MODE == "real" and exchange is None:
                    continue
                if exchange is None and TRADE_MODE == "real":
                    continue
                # If exchange not available and we are in virtual mode, we still skip fetch_ohlcv
                if exchange is None:
                    continue
            ...
            for trade in snapshot:
                try:
                    ...
                    if exchange:
                        ...
                        # If no real exchange, skip monitoring (virtual trades won't move)
                        continue
                    if direction == "LONG":
...
async def callback_query_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("tf_toggle:"):
        ...
        if tf in ACTIVE_TF:
            ACTIVE_TF.remove(tf)
            await query.edit_message_text("TF toggled: " + ", ".join(ACTIVE_TF))
        else:
