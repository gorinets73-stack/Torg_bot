...
# Telegram (put in env preferably)
...
# ==== EXCHANGE CLIENT ====
# -------------------------
# initialize exchange (public methods work without keys)
exchange = None
try:
    if BITGET_API_KEY and BITGET_API_SECRET:
        ...
        # initialize unauthenticated client for market data
        exchange = ccxt.bitget({
            "enableRateLimit": True,
        })
        exchange.load_markets()
...
def place_real_market_order(symbol, side, amount):
    try:
        # ccxt.create_order(signature may vary per exchange; using create_order)
    ...
    for symbol in SYMBOLS:
        for tf in list(ACTIVE_TF):
            try:
                ...
                # respect trend: if trend_up only allow LONG (configurable)
                if direction == "LONG" and not trend_up:
                    # skip LONG in downtrend
                    continue
                if direction == "SHORT" and trend_up:
                    # skip SHORT in uptrend
                    continue

                if direction:
                    # check if already open for symbol/timeframe
                    existing = find_existing_trade_for(symbol, tf)

                    # If opposite signal and existing open -> close and (optionally) open new
                    if existing:
                        if existing["direction"] != direction:
                            # close existing at market price
                            # if real -> attempt to close real position
                            ...
                            if TRADE_MODE == "virtual" or not (BITGET_API_KEY and BITGET_API_SECRET):
                                ...
                                # attempt to set leverage if supported (best-effort)
                                try:
                        ...
                        # no existing trade -> open new if funds ok
...
def monitor_open_trades():
    for trade in list(open_trades):
        try:
            ...
            logging.error(f"Ошибка мониторинга {trade.
