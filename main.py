...
SYMBOLS = [
    ...
    "TRX/USDT",   # TRON
    "DOT/USDT",   # Polkadot
    "MATIC/USDT", # Polygon (старое имя, бот заменит на POL/USDT)
    "AVAX/USDT",  # Avalanche
    "LINK/USDT",  # Chainlink
    "LTC/USDT",   # Litecoin
...
def fix_symbols(symbols):
    ...
    for s in symbols:
        try:
            real = get_symbol(s)
            if real != s:
                logging.info(f"🔄 Заменяю {s} → {real}")
            fixed.append(real)
        except Exception as e:
            logging.error(f"❌ Символ {s} пропущен ({e})")
    return fixed

# Исправляем список перед стартом
SYMBOLS = fix_symbols(SYMBOLS)
...
def analyze_symbol(symbol):
    try:
        ...
        if rsi < 30 and price > sma50:
            return f"🟢 LONG {symbol} {INVEST_AMOUNT}$ (RSI={round(rsi,2)}, цена выше тренда)"
        elif rsi > 70 and price < sma50:
            return f"🔴 SHORT {symbol} {INVEST_AMOUNT}$ (RSI={round(rsi,2)}, цена ниже тренда)"
        else:
            return None
    except Exception as e:
