...
def size_from_usd(price, amount_usd, leverage):
    return float(round((amount_usd * leverage) / price, 6))
                ...
                if direction:
                    ...
                    if existing:
                        if existing["direction"] != direction:
                            ...
                            if TRADE_MODE == "virtual" or not (BITGET_API_KEY and BITGET_API_SECRET):
                                if not virtual_reserve(INVEST_AMOUNT):
                                    send_message(CHAT_ID, f"⚠️ Недостаточно виртуального баланса для {symbol}.")
                                    continue
                                ...
                                if order:
                                    ...
                                    send_message(CHAT_ID, f"⚠️ Ошибка открытия реальной сделки {symbol}")
                        else:
                            continue
                    else:
                        ...
                        if TRADE_MODE == "virtual" or not (BITGET_API_KEY and BITGET_API_SECRET):
                            ...
                            try:
                                amount = size_from_usd(price, INVEST_AMOUNT, LE
