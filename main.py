...
def close_trade(trade, exit_price, reason):
    ...
    if not trade.get("real"):
        # PnL cash is profit (can be negative), add back invest + pnl
        # Note: previously we reserved INVEST_AMOUNT from available; now we return invest + pnl to total & available.
        invest = trade["invest"]
        pnl_cash = trade.get("pnl_cash", 0.0)
        # return initial invest and pnl
        virtual_release(invest + pnl_cash)
    else:
        # real: no virtual balance change
        pass
...
def check_signals_once():
    for symbol in SYMBOLS:
        for tf in ACTIVE_TF:
            try:
                if exchange is None:
                    # если нет exchange, нельзя fetch_ohlcv -> пропускаем сигналный цикл
                    continue
                ...
                if direction:
                    ...
                    if TRADE_MODE == "virtual" or not exchange:
                        # резервируем виртуальные средства; если не хватает — пропускаем
    ...
    for trade in list(open_trades):
        try:
            ...
            if exchange:
                ...
                # если нет биржи — не можем автоматически закрывать (виртуал)
                continue
    ...
    if request.method == "POST":
        ...
        if "message" in data:
            ...
            if text == "/start":
                ...
                if row:
                    kb["inline_keyboard"].append(row)
                kb["inline_keyboard"].append([{"text":"Set Virtual","callback_data":"mode_virtual"},{"text":"Set Real","callback_data":"mode_real"}])
